...
loader.load(
  ...
  gltf => {
    ...
    footMesh.geometry = footMesh.geometry.toNonIndexed(); // Ensure access to individual vertices
    const positionAttr = footMesh.geometry.attributes.position;
    const vertexCount = positionAttr.count;

    // Initialize pain levels and location mapping
    for (let i = 0; i < vertexCount; i++) {
      painLevels[i] = 0;
      locationMap[i] = Math.floor(i / (vertexCount / locationDescriptions.length)); // crude mapping
    }
